...
\begin{itemize}
  ...
  \item The cslab-nodes are located inside a virtual private network. These internal nodes are only SSH accessible via the \verb|cslab-bastion.cs.wichita.edu| jumphost. Any external connection into the cslab environment must proxy connect through the \verb|cslab-bastion|.
  \item \verb|cslab-bastion.cs.wichita.edu| and \verb|cslab-sftp.cs.wichita.edu| can be used as SCP or SFTP file-server hosts with SSH clients, such as \textit{OpenSSH}, or graphical SFTP clients, such as \textit{Filezilla}.
...
\begin{enumerate}
  ...
  \item If everything is correctly configured, a new \textit{PuTTY} terminal emulator window will open and automatically log you into an available cslab node! You should be presented with a standard shell prompt within the node: \break
...
\subsection*{Automatically adding the cslab host keys to your registry for \textit{PuTTY} to use:}
\begin{itemize}
  ...
  \item The \verb|cslab_putty_ssh_client_full_configuration| file from \href{https://github.com/benroose/tutorials/tree/master/cslab_ssh_client_config_files}{Ben Roose's GitHub tutorials repository} will automatically add the cslab host keys into your Windows registry when you add this file during the initial configuration and set up of \textit{PuTTY}. However, if your configuration loses its cached host keys for any reason, then it may ask you to reconfirm the authenticity of cslab hosts when you try to open an SSH connection.
  \item You can also download the \verb|cslab_putty_ssh_client_host_keys_only| file from \href{https://github.com/benroose/tutorials/tree/master/cslab_tutorials/cslab_ssh_client_config_files}{Ben Roose's GitHub tutorials repository} and re-add the host keys into your Windows registry.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Download the file \verb|cslab_openssh_client_config| from \href{https://github.com/benroose/tutorials/tree/master/cslab_tutorials/cslab_ssh_client_config_files}{Ben Roose's GitHub tutorials repository}.
...
\end{verbatim}
  \item Alternatively, you can manually copy the following host entry into your local user's \verb|~/.ssh/config| file:
\end{enumerate}
...
\begin{enumerate}
  \setcounter{enumi}{3}
  \item The cslab SSH host keys must be then added into your local user's \verb|~/.ssh/known_hosts| file. cslab host keys are available for download in the file \verb|cslab_openssh_known_hosts| from \href{https://github.com/benroose/tutorials/tree/master/cslab_tutorials/cslab_ssh_client_config_files}{Ben Roose's GitHub tutorials repository}.
...
\begin{enumerate}
  ...
  \item Connect to the cslab Linux environment by typing \break
    \verb|ssh cslab|
  \item If everything is correctly configured, you will be automatically connected to an available cslab node! You should be presented with a standard shell prompt within the node: \break
    \verb|your_mywsu_id@cslab-node-#:~$|
...
\begin{itemize}
  ...
  \item \textbf{Only connect to the last used cslab-node if you need access into a previously running SSH session. For normal use the best option is always let the ballast load-balancer automatically connect to an available node.}
...
\begin{itemize}
  ...
  \item You can download the file \verb|cslab_openssh_known_hosts| from \href{https://github.com/benroose/tutorials/tree/master/cslab_tutorials/cslab_ssh_client_config_files}{Ben Roose's GitHub tutorials repository}.
  \item The cslab host keys can then be re-added into your local user's \verb|known_hosts| by changing into the directory where you downloaded the \verb|cslab_openssh_known_hosts| file and typing \break
    \verb|cat cslab_openssh_known_hosts >> ~/.ssh/known_hosts|
\end{itemize}
