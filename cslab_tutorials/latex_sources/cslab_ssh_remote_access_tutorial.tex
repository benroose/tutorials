...
%%   \item Open a Windows command prompt/shell.
%%   \item To copy a file from your local computer to your user home directory on cslab using \textit{PuTTY Secure Copy (PSCP)}, type \break
%%   \verb|pscp local_filename_or_path cslab-sftp:~|
...
%%   \verb|pscp cslab-sftp:~/remote_filename_or_path local_directory|
    
%%   \item To use the \textit{PuTTY SSH File Transfer Protocol (PSFTP)} interactive command-line tool, type \break
%%   \verb|psftp cslab-sftp|  
...
\begin{itemize}
  ...
  \item The \verb|cslab_putty_ssh_client_full_configuration| file from \href{https://github.com/benroose/tutorials/tree/master/cslab_ssh_client_config_files}{Ben Roose's GitHub tutorials repository} will automatically add the cslab host keys into your Windows registry when you add this file during the initial configuration and set up of \textit{PuTTY}. However, if your configuration lost the host keys for any reason, then it may ask you to confirm the authenticity of cslab hosts when you try to open an SSH connection.
...
\begin{enumerate}
  \item Install the \texttt{openssh-client} software package using your distribution's package management tools.
  \item On your Linux or Mac desktop/laptop computer, copy the following host entry into your local user's \verb|~/.ssh/config| file:
\end{enumerate}
...
Host cslab-sftp cslab-sftp.cs.wichita.edu
  ...
  HostKeyAlias cslab.cs.wichita.edu
\end{verbatim}
(Note: replace \verb|your_mywsu_id| with your own 8 character ID number in both \texttt{ProxyCommand} and \texttt{User} lines.)

\begin{enumerate}
  \setcounter{enumi}{1}
  \item The above text is also available for download in the file \texttt{cslab\_openssh\_client\_config} from \href{https://github.com/benroose/tutorials/tree/master/cslab_tutorials/cslab_ssh_client_config_files}{Ben Roose's GitHub tutorials repository}.
  \item Open a command-line terminal emulator window and generate a new \textit{OpenSSH} public/private key pair for your local user by typing \break
