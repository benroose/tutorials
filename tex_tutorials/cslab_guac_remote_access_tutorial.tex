...
\item When you first connect into an RDP desktop session, occasionally you may see a policykit error message pop up. This is a known software bug which is being worked on. Clicking the OK button will close the error and it should not affect the rest of your login session.
...
\question{How many web-browser tabs/windows can I open into the remote environment? }\label{multiple bowser tabs}
...
\item You can open one web-browser tab/window to connect to a graphical RDP desktop session and open additional tabs/windows to connect to SSH terminal sessions concurrently. However, the \textit{Apache Guacamole} system cannot guarantee that your RDP desktop session and your SSH terminal session(s) will connect to the same remote cslab server.
\end{itemize}

%------------------------------------------------

\question{How do I access the cslab Linux environment on port 22 via an SSH client?}\label{ssh client}

\textbf{Please only connect directly into the cslab environment with an SSH client if you have previous experience in using SSH and the Linux command-line. If you are new to Linux, please use the \textit{Guacamole} web-browser interface as discussed above.}

\subsection*{Using \textit{OpenSSH} client on Linux or Mac OSX:}
\begin{enumerate}
\item Add the following host entry into your ~/.ssh/config file:
\begin{verbatim}
Host cslab-cluster cslab-cluster-last cslab-cluster.cs.wichita.edu cslab-cluster-last.cslab-cluster.cs.wichita.edu
    ProxyCommand ssh cslab-bastion /usr/local/bin/ballast %h
    HostKeyAlias cslab-cluster.cs.wichita.edu
\end{verbatim}
