...
\newlistof{questions}{faq}{\large FAQ for web-access to the cslab Linux environment}
...
\question{How do I access the cslab Linux environment via a web-browser?}\label{login}
...
\centering
\end{figure}
\item To connect into an LXDE graphical desktop session for working on programming assignments click on \texttt{CSLAB\_RDP}.
\item To connect into a command-line terminal session for working on programming assignments click on \texttt{CSLAB\_SSH}.
...
\question{What software is available within the cslab Linux environment?}\label{software}

The cslab environment gives you both graphical and command-line Linux tools for writing, compiling, and debugging your CS programming class assignments. Software tools/packages installed on the cslab servers includes:
\begin{itemize}
\item Text and code editors: leafpad, nano, vim, and emacs.
\item Integrated development environments (IDE): atom, geany, and eclipse.
\item Compiling tools: GNU C compiler (gcc), g++, make, perl, python, python3, and java.
...
\end{itemize}
To check if a specific software package or version is installed within the cslab servers, connect into the SSH terminal session or open a terminal emulator in the RDP desktop session and type:
\begin{verbatim}
apt list --installed specify_package_name_here
\end{verbatim}

If a Linux software package is not installed within the cslab environment which you require to complete your class assignment, then please ask your instructor whether this package can be installed for you by the EECS systems administrator.

\textbf{Do not attempt to install any software packages yourself!}

%------------------------------------------------

\question{How do I copy text within the remote cslab Linux environment? }\label{copying text}
...
\question{How do I download/upload files within the cslab Linux environment? }\label{upload/download files}
...
\question{How many tabs/windows can I open into the remote environment? }\label{limitations}

\begin{itemize}
\item A restriction within the RDP remote desktop software only allows a single connection per user, which means you can only view the remote RDP desktop session from one web-browser tab/window. If you attempt to open more than one web-browser tab/window and connect to multiple RDP desktop sessions, then the gateway will disconnect from one tab and reconnect in another tab.
\item A restriction within the SSH software allows a maximum of six concurrent connections per user, which means you can open up to six web-browser tabs/windows into remote SSH terminal sessions at the same time. Each open SSH terminal session tab/window will run a separate Bash shell instance on the same remote cslab server.
\item You can also use the \texttt{tmux} or \texttt{screen} terminal multiplexer commands to run multiple command-line shells concurrently.
\item You can open one web-browser tab/window to connect to an RDP desktop session and open one or more additional tabs/windows to connect to SSH terminal sessions concurrently. However, there is no guarantee that your RDP desktop session and your SSH terminal session(s) will connect to the same remote cslab server.

  We still have a slight glitch with policykit, which sometimes pops up an error message when a user first logs into an RDP session. Clicking the OK button will close the error and it shouldn't affect the student's work for the rest of that login session.
\end{itemize}
