...
\begin{itemize}
  ...
  \item If you cannot access the cslab environment using your myWSU ID, please contact the EECS systems administrator, \href{mailto:ben.roose@wichita.edu}{ben.roose@wichita.edu}.
\end{itemize}
...
(where \texttt{grader\_account} is the class grader account, i.e, \texttt{cs211a}).
  \item Use standard Linux command-line tools to access, view, and compile student assignments, i.e. \texttt{ls, cd, less, vi, emacs, gcc, g++}.
  \item You will find each student assignment in the directory \break
...
\texttt{ssh -X cs211a@localhost}
  \item To use a graphical application type the application's name followed by an ampersand (\&) in the terminal emulator, i.e.
    \texttt{geany \&} \break
    
\textbf{NOTE: The -X option for graphical applications can ONLY be used within an RDP desktop session.}
\end{enumerate}
\end{flushleft}
